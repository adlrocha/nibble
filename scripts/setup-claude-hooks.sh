...
USERPROMPT_CMD='if [ -n "$AGENT_TASK_ID" ]; then INPUT=$(cat); if command -v jq >/dev/null 2>&1; then MSG=$(printf "%s" "$INPUT" | jq -r ".message // empty"); [ -n "$MSG" ] && nibble memory capture "$AGENT_TASK_ID" "user" "$MSG" 2>/dev/null || true; fi; fi'

# PostToolUse — captures tool calls + results into the memory capture JSONL.
POSTTOOL_CMD='if [ -n "$AGENT_TASK_ID" ]; then INPUT=$(cat); if command -v jq >/dev/null 2>&1; then TOOL=$(printf "%s" "$INPUT" | jq -r ".tool_name // empty"); TOOL_INPUT=$(printf "%s" "$INPUT" | jq -c ".tool_input // {}" | cut -c1-4096); TOOL_OUTPUT=$(printf "%s" "$INPUT" | jq -r ".tool_output // \"\"" | cut -c1-4096); [ -n "$TOOL" ] && nibble memory capture "$AGENT_TASK_ID" "tool" "" --tool-name "$TOOL" --tool-input "$TOOL_INPUT" --tool-output "$TOOL_OUTPUT" 2>/dev/null || true; fi; fi'
