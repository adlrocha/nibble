...
You are running inside an isolated Podman sandbox managed by **nibble**. This file
contains all instructions for how to operate inside this environment. Read it fully
...
| `yarn.lock` | `yarn install` | `yarn start / yarn test` |
| `pnpm-lock.yaml` | `pnpm install` | `pnpm start / pnpm test` |
| `Cargo.toml` | `cargo build` | `cargo run / cargo test` |
| `go.mod` | `go mod download` | `go run . / go test ./...` |
| `requirements.txt` | `pip install -r requirements.txt` | `python main.py / pytest` |
...
Load the skill for each stage before executing it. Skills are available via the skill
tool. Start with the pipeline manifest to understand the full flow:
